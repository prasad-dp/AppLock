...
class AppLockAccessibilityService : AccessibilityService() {
    ...
    override fun onServiceConnected() {
        ...
        // Immediately load locked packages from database to guarantee 0ms protection on start
        serviceScope.launch(Dispatchers.IO) {
            try {
                val initialList = repository.getAllLockedApps().filter { it.isLocked }.map { it.packageName }
                lockedPackages.addAll(initialList)
                Log.d(TAG, "Accessibility initial locked packages loaded: ${lockedPackages.size}")
            } catch (e: Exception) {
        ...
        if (AppLockPackageHelper.isLauncherPackage(this, pkgName)) {
            ...
            for (unlockedApp in currentUnlockedApps) {
                ...
                if (outOfForegroundDuration >= relockThresholdMs) {
                    ...
                    Log.d(TAG, "Accessibility: Auto-relocked app $unlockedApp on Home Launcher after ${outOfForegroundDuration}ms (Policy: $relockPolicy)")
                }
            }
            return
        }
